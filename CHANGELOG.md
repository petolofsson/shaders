...
- **R143 highlight reconstruction** (`inverse_grade.fx`) — Always-on chroma rolloff in Oklab for near-SDR-clipped highlights. Fires before R90 expansion. No knob — physically correct defect correction, not a stylistic choice. Two self-limiting smoothstep gates: near-clip zone `smoothstep(0.88, 0.995, max_ch)` (wide for 8-bit quantization — ~10 linear levels) and C gate `smoothstep(0.18, 0.08, C)` (protects intentionally saturated colored lights). Desaturates only — preserves hue direction, never introduces color, no extra passes.
