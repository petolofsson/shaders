...
- **R143 highlight reconstruction** (`inverse_grade.fx`) — Chroma rolloff in Oklab for near-SDR-clipped highlights. Fires before R90 expansion. Two self-limiting smoothstep gates: near-clip zone `smoothstep(0.88, 0.995, max_ch)` (wide for 8-bit quantization — ~10 linear levels) and C gate `smoothstep(0.18, 0.08, C)` (protects intentionally saturated colored lights). Desaturates only — preserves hue direction, never introduces color, no extra passes. Non-overlapping with R90 (which fires on mid-luminance, high-C pixels; reconstruction fires on high-L, low-C pixels). `HIGHLIGHT_RECONSTRUCT 0.00` (off, awaiting calibration; start 0.5–0.8).
