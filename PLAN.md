...
No game post-processing pipeline attempts to undo tonemapper compression — that domain claim is solid. The data highway architecture (HighwayTex 256×1 R16F + TexHwyTex 2D spatial lane + data rows) is not a pattern in game post-processing — prior art survey (R203) confirmed no matching pattern in real-time post-process literature. Per-hue HueCeil ceilings from Munsell Renotation data, FilmCurve pre-inverse (`FilmCurveInvCh`) applied before chroma expansion — not found in games. R198: piecewise exact FilmCurve inverse with histogram-derived knee per frame — genuinely novel in real-time. R203: unified texture highway (TexHwyTex) — 2D extension of scalar highway with resolution-independent spatial lane and packed data rows; declared once in common.fxh; pass-through write mechanism — no identical pattern found in post-process literature.
...
**Stage 1 — Film Stock (92% novel)**
Games approximate film stocks with LUTs — even RDR2's celebrated film look is LUT-based. The 2383 3×3 spectral dye matrix from H-1-2383t primary data, DIR couplers, chromatic floor, masking coupler, bleach bypass — none of this exists in any game post-processing pipeline. R192: PRINT_STOCK/BLEACH_BYPASS in `ApplyLook` post-chroma — correct physical LMT placement. R202: Kodak Vision3 500T negative spectral sensitivity matrix (M_neg) — completes the physical two-stock chain (input negative sensitivity + print dye masking). No game post-process pipeline implements the negative-side matrix. Derived from H-1-5219t datasheet via Gotanda 2010 sampling method.

R204 (2026-05-19): FilmCurveApply removed. Research confirmed the adaptive rational S-curve (histogram-mode-derived knee, shoulder, toe) was physically unjustified in display-referred space — toe modeled a photochemical process that ACES already handles upstream; shoulder was redundant given WHITES remap. Shadow/highlight treatment now fully owned by PRINT_STOCK and BLACKS/WHITES. DIR_COUPLER and M_NEG promoted from hardcoded to user-exposed knobs.

Drag reducing score: (1) 3-way CC exists in UE5 and most real-time grading stacks — not novel. (2) Removal of FilmCurveApply eliminates the histogram-adaptive tone curve — a novelty claim that no longer applies. (3) DIR-adjacent couplers exist in desktop photo tools (DxO FilmPack, Silver Efex) — novel in real-time games but not broader tooling. (4) M_neg off-diagonal values are small (max 2.45%) — method is novel but visual contribution is subtle by construction.

Remaining 8%: BLEACH_BYPASS shadow desaturation calibration pending visual evaluation against real footage references. Print stock desat_w calibration (fc_knee_toe pivot) is empirical.
...
Drag reducing score: (1) Halation removed — 2D framebuffer post-process cannot distinguish light sources from lit surfaces; correct scatter requires engine-side light graph access. (2) Hue rotation, saturation, and vibrance knobs exist in UE5 and every real-time grading stack — 6-band per-hue control is more granular than most games but the concept is not new. (3) Shadow cast (bipolar warm/cool) is a tinting operation — conceptually not different from a shadow CC wheel with two modes. (4) The three scene-adaptive effects (HK magnitude, Abney scale) are each individually literature-grounded but the adaptation constants are empirically calibrated, not derived from display-referred psychophysical measurements.

Remaining 2%: Abney and HK scale calibration against display-referred measurements is unresolved.
