...
float4 CaptureLumaHistPixel(int px)
{
    float samples[64];
    float mean = 0.0;
    ...
    float bucket_lo = float(px)     / float(SCOPE_BINS);
    float bucket_hi = float(px + 1) / float(SCOPE_BINS);
    float count = 0.0;
    [loop] for (int i = 0; i < SCOPE_S * SCOPE_S; i++)
        count += (samples[i] >= bucket_lo && samples[i] < bucket_hi) ? 1.0 : 0.0;
    float v = count / float(SCOPE_S * SCOPE_S);
    return float4(v, v, v, 1.0);
}

float4 CaptureHueHistPixel(int b)
{
                      ...
                      float2 uv  : TEXCOORD0) : SV_Target
{
    if (pos.y < 1.0 && int(pos.x) <= SCOPE_BINS)
        return CaptureLumaHistPixel(int(pos.x));
    if (pos.y < 1.0 && int(pos.x) >= HUE_OFFSET && int(pos.x) < HUE_OFFSET + HUE_BINS)
        return CaptureHueHistPixel(int(pos.x) - HUE_OFFSET);
