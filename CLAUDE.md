...
**DiffusionDownsample + DiffusionBlurH + DiffusionBlurV + DiffusionPS passes (same technique):** Diffusion merged into grade.fx. DiffusionDownsamplePS: 4-tap box → DiffusionTex (1/4-res). DiffusionBlurHPS: 9-tap horizontal Gaussian (σ=2 output texels) → DiffusionHorizTex. DiffusionBlurVPS: 9-tap vertical Gaussian → DiffusionTex (final blurred source). DiffusionPS composites via HBM dual-component model: A) additive shimmer `max(0, diff_blur − base) * src_gate * adapt_str` — highlight scatter only; B) soft midtone overlay `lerp(result, diff_blur, eff_diff * 0.06 * mid_gate)` — bell-gated, zero at blacks/whites. Radial vertical oval (xs=1.6, ys=0.08): full clarity top-to-bottom, diffusion increases left/right. **R136 film grain** at end of DiffusionPS: pcg3d RGB-decorrelated noise (R:G:B = 1.00:0.80:1.50), Selwyn 2383 amplitude envelope `σ = GRAIN_STRENGTH × 0.018 × sqrt(1 − L_gamma)`, framerate-independent via `grain_slot = uint(FRAME_COUNT × (FRAME_TIME / 41.667))` — turns over at ~24fps at any display fps. Uniforms: `FRAME_COUNT` (framecount) and `FRAME_TIME` (frametime ms) declared in grade.fx.

**Data highway (HighwayTex, 256×1 R16F):** x=194–196 p25/p50/p75 · x=197 chroma expansion slope (IQR+Bowley, encode (v−1)/1.5) · x=198 median Oklab C (histogram p50, raw [0,0.30]) · x=199 scene cut · x=200 p90 · x=201 mean chroma angle (atan2 encoded) · x=202 achromatic fraction · x=203 zone_key · x=204 zone_std · x=205 slow_key · x=206 luma histogram mode (EMA-smoothed)
